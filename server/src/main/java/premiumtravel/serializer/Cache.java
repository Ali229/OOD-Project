...
public final class Cache {
	...
	@PostConstruct
	void init() {
		setProtocol( SerializationProtocol.valueOf( ini.get( "Serializer", "serializer" ) ) );
	 ...
	 * data using the assigned serialization protocol or with the default serializer if there's an error.
	 */
	@PreDestroy
	void tearDown() throws IOException {
		if ( this.serializer == null ) {
			logger.error(
					"The system is shutting down while no serializer is set. The data will be serialized with the DEFAULT serializer." );
			Date date = new Date();
	...
	public void setProtocol( SerializationProtocol protocol ) {
		...
		try {
			...
			logger.error( "The new " + this.protocol.name() + " serializer could not be created." );
		}
	}
